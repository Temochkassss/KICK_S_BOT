...
class ShowAdminResponses(
    ...
    override fun name(): String = "admin_view_responses"
    ...
    override fun handleCallback(update: Update) {
        ...
        showAdminResponses(chatId)
    }

    fun showAdminResponses(chatId: Long, page: Int = 1) {
        val pageSize = 5
        ...
        val responses = responseHelper.getAllResponses (page, pageSize)

        messageHandlerUtils.handleMessageEdit(
            chatId = chatId,
            newText = "🗂 Для просмотра откликов — выберите:",
            inlineKeyboard = inlineKeyboardTemplates.getAdminResponsesIK(responses, page, totalPages),
            logger = logger
        )
    }
}